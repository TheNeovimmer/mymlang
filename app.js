...
let state = { route: 'welcome', mode: 'text', signQuery: '', selectedSign: signs[0], toast: '', cameraStream: null, videoUrl: '' };
...
function header(title, subtitle = '') {
  return `<header class="app-header">
    <button class="icon-button" data-route="home" aria-label="Go home">${icon('chevron')}</button>
    <div class="brand">
...
function home() {
  return shell(`<section>
    ...
    <div class="history-list"><div class="history-item"><div><b>Thank you</b><small>Text to sign, 8 minutes ago</small></div>${icon('chevron')}</div><div class="history-item"><div><b>Good morning</b><small>Voice to sign, yesterday</small></div>${icon('chevron')}</div></div>
...
function translate() {
  return shell(`<section>
    ...
    <div class="mode-tabs" role="tablist">
      ${['text','voice','video'].map(m => `<button class="mode-tab ${state.mode === m ? 'active' : ''}" data-mode="${m}">${m}</button>`).join('')}
    </div>
...
function dictionary() {
  const filtered = signs.filter(s => `${s[1]} ${s[2]} ${s[0]}`.toLowerCase().includes(state.signQuery.toLowerCase()));
  return shell(`<section>
    ...
    <div class="search-wrap">${icon('search')}<input class="search-input" id="signSearch" value="${state.signQuery}" placeholder="Search for a sign..." aria-label="Search dictionary"></div>
    <div class="filter-tabs"><button class="filter-tab active">All signs</button><button class="filter-tab">Recent</button><button class="filter-tab">Favorites</button></div>
    ...
    ${state.selectedSign ? signDetail(state.selectedSign) : ''}
  </section>`, 'Dictionary', 'visual sign guide');
}
function signRow(s, i) {
  return `<button class="sign-item" data-sign-index="${signs.indexOf(s)}"><span class="sign-letter">${s[0]}</span><span><strong>${s[1]}</strong><span>${s[2]}</span></span><span class="favorite">${s[3] ? icon('star') : icon('chevron')}</span></button>`;
}
function signDetail(s) {
  return `<article class="sign-detail-card"><div class="sign-video-box">${icon('play')}</div><h2 class="section-heading" style="margin-top:0">${s[1]}</h2><p class="page-subtitle">${s[2]} Practice slowly first, then repeat at conversation speed.</p><button class="primary-btn" data-practice-sign="${s[1]}">Practice this sign</button></article>`;
}
function lessonList(list = lessons) {
  return `<div class="lesson-list">${list.map(l => `<button class="lesson-card" data-route="lessons"><div class="lesson-progress">${l[2]}%</div><div><h3>${l[0]}</h3><p>${l[1]}</p><div class="progress-line" style="--value:${l[2]}%"><i></i></div></div></button>`).join('')}</div>`;
}
...
function profile() {
  return shell(`<section><div class="profile-top"><img class="avatar" src="logo.png" alt="Karim profile avatar"><h1 class="page-title">Karim daldoul</h1><div class="rank">Beginner Explorer, level 12</div></div><div class="stats-grid"><div class="mini-stat"><b>142</b><span>words</span></div><div class="mini-stat"><b>7</b><span>day streak</span></div><div class="mini-stat"><b>12h</b><span>practice</span></div></div><div class="progress-line" style="--value:75%"><i></i></div><h2 class="section-heading">Achievements</h2><div class="achievement-row">${['First word','Verified','Master','Scholar','Social'].map((a,i)=>`<div class="achievement ${i>1?'locked':''}"><div class="badge-icon">${icon(i>1?'star':'trophy')}</div><small>${a}</small></div>`).join('')}</div><h2 class="section-heading">Account</h2><div class="settings-list"><button class="settings-item" data-route="settings"><span class="settings-left"><span class="icon-wrap">${icon('user')}</span><span>Account information<small>Name, email and accessibility preferences</small></span></span>${icon('chevron')}</button><button class="settings-item" data-route="settings"><span class="settings-left"><span class="icon-wrap">${icon('bell')}</span><span>Notifications<small>Practice reminders and lesson updates</small></span></span>${icon('chevron')}</button></div></section>`, 'Profile', 'progress and settings');
}
function settings() {
  return shell(`<section><h1 class="page-title">Settings</h1><p class="page-subtitle">Tune the app for comfortable learning and accessible translation.</p><div class="settings-list"><div class="settings-item"><span class="settings-left"><span class="icon-wrap">${icon('volume')}</span><span>Speak translations<small>Read text output aloud</small></span></span><span class="toggle"></span></div><div class="settings-item"><span class="settings-left"><span class="icon-wrap">${icon('camera')}</span><span>Camera permissions<small>Used only for live sign recognition</small></span></span>${icon('chevron')}</div><div class="settings-item"><span class="settings-left"><span class="icon-wrap">${icon('mail')}</span><span>Support<small>help@mym-sign.app</small></span></span>${icon('chevron')}</div></div><h2 class="section-heading">Legal</h2><div class="history-list"><div class="history-item"><b>Privacy policy</b>${icon('chevron')}</div><div class="history-item"><b>Terms of service</b>${icon('chevron')}</div></div></section>`, 'Settings', 'accessibility');
}
function notFound() {
...
function routeView() {
  return ({ welcome, home, translate, dictionary, lessons: lessonsPage, practice, profile, settings }[state.route] || notFound)();
}
function render() {
...
function bindEvents() {
  ...
  if (search) search.addEventListener('input', e => { state.signQuery = e.target.value; render(); });
